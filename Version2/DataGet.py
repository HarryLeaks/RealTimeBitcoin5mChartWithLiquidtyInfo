...
def updateLiquidations():
    ...
    global liquidations
    long1 = 0; short1 = 0; long4 = 0; short4 = 0; long12 = 0; short12 = 0; long24 = 0; short24 = 0
    while True:
        long1, short1 = getLiquitations(url1)
        long4, short4 = getLiquitations(url4)
        long12, short12 = getLiquitations(url12)
        long24, short24 = getLiquitations(url24)
        liquidations = [long1, short1, long4, short4, long12, short12, long24, short24]
...
def get_open_interest_long_short_ratio(symbol, period, limit):
    ...
    topAccount_url = f'{base_url}/futures/data/topLongShortAccountRatio?symbol={symbol}&period={period}&limit={limit}'
    ...
    topAccount_response = requests.get(topAccount_url)
    ...
    if (
        ...
        if (
            ...
            and len(topAccount_data) > 0
        ):
            long_short_ratio = ratio_data[0]['longShortRatio']
            top_long_short_ratio = topAccount_data[0]['longShortRatio']
    ...
    return long_short_ratio, open_interest, taker_buy_volume, taker_sell_volume, top_long_short_ratio
...
def handle_trade(trade, symbol):
    global df, liquidity, cumulative_bids, cumulative_asks, liquidations
    ...
    trade_data = {
        ...
        'top_long_short_ratio': liquidity[4],
        'long1H': liquidations[0],
        'short1H': liquidations[1],
        'long4H': liquidations[2],
        'short4H': liquidations[3],
        'long12H': liquidations[4],
        'short12H': liquidations[5],
        'long24H': liquidations[6],
        'short24H': liquidations[7]
    }

    new_trade_data = []  # List to store new trade data

    # Add cumulative_bids and cumulative_asks as new columns
    for i in range(100):
        column_name_bids = f'cumulative_bids_{i}'
        column_name_asks = f'cumulative_asks_{i}'
        trade_data[column_name_bids] = cumulative_bids[i]
        trade_data[column_name_asks] = cumulative_asks[i]

    new_trade_data.append(trade_data)

    # Concatenate new trade data with the existing DataFrame
    if new_trade_data:
        ...
        print(liquidations[0], liquidations[1], liquidations[2], liquidations[3], liquidations[4], liquidations[5], liquidations[6], liquidations[7])
...
def update_chart(frame, ax1, ax2, ax3):
    ...
    ax3.plot(data['timestamp'], data['long_short_ratio'], color='orange', label='Long/Short Ratio')
    ax3.plot(data['timestamp'], data['top_long_short_ratio'], color='green', label='Top Long/Short Ratio')
...
if __name__ == "__main__":
    ...
    thread3 = threading.Thread(target=updateLiquidations, args=())
    thread3.start()
